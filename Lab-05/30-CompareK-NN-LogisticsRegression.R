...
# Bước 9: Kết luận và so sánh ưu nhược điểm

cat("\n===== KẾT LUẬN VÀ SO SÁNH ƯU NHƯỢC ĐIỂM =====\n\n")

cat("1. So sánh hiệu suất:\n")
better_acc <- ifelse(conf_matrix_knn$overall["Accuracy"] > conf_matrix_logit$overall["Accuracy"], 
                     "K-NN", "Logistic Regression")
cat("   - Về độ chính xác tổng thể:", better_acc, "tốt hơn\n")

better_sens <- ifelse(conf_matrix_knn$byClass["Sensitivity"] > conf_matrix_logit$byClass["Sensitivity"], 
                      "K-NN", "Logistic Regression")
cat("   - Về độ nhạy (khả năng phát hiện người bệnh):", better_sens, "tốt hơn\n")

better_spec <- ifelse(conf_matrix_knn$byClass["Specificity"] > conf_matrix_logit$byClass["Specificity"], 
                      "K-NN", "Logistic Regression")
cat("   - Về độ đặc hiệu (khả năng phát hiện người không bệnh):", better_spec, "tốt hơn\n")

better_auc <- ifelse(auc_knn > auc_logit, "K-NN", "Logistic Regression")
cat("   - Về AUC-ROC:", better_auc, "tốt hơn\n")

cat("\n2. Ưu điểm của K-NN:\n")
cat("   - Mô hình không có giả định về dữ liệu\n")
cat("   - Đơn giản, dễ hiểu và triển khai\n")
cat("   - Có thể bắt được mối quan hệ phi tuyến tính trong dữ liệu\n")
cat("   - Hoạt động tốt với tập dữ liệu nhỏ và vừa\n")

cat("\n3. Ưu điểm của Logistic Regression:\n")
cat("   - Cung cấp giải thích rõ ràng về ảnh hưởng của từng đặc trưng (thông qua odds ratio)\n")
cat("   - Hiệu quả tính toán cao, đặc biệt với dữ liệu lớn\n")
cat("   - Ít nhạy cảm với overfitting hơn so với K-NN\n")
cat("   - Dễ dàng cập nhật mô hình với dữ liệu mới\n")

cat("\n4. Nhược điểm của K-NN:\n")
cat("   - Chi phí tính toán cao khi dự đoán (phải tính khoảng cách đến mọi điểm trong tập huấn luyện)\n")
cat("   - Nhạy cảm với nhiễu và dữ liệu outlier\n")
cat("   - Cần lưu trữ toàn bộ dữ liệu huấn luyện\n")
cat("   - Hiệu suất giảm khi số lượng đặc trưng tăng (curse of dimensionality)\n")

cat("\n5. Nhược điểm của Logistic Regression:\n")
cat("   - Giả định mối quan hệ tuyến tính giữa các đặc trưng và logarit của odds\n")
cat("   - Khó bắt được mối quan hệ phức tạp, phi tuyến tính\n")
cat("   - Yêu cầu nhiều dữ liệu để ước lượng chính xác các tham số\n")
cat("   - Nhạy cảm với sự tương quan cao giữa các đặc trưng (multicollinearity)\n")

cat("\n6. Kết luận chung:\n")
cat("   - Logistic Regression cung cấp hiểu biết sâu hơn về ảnh hưởng của từng đặc trưng\n")
cat("   - K-NN có khả năng bắt được các mối quan hệ phức tạp hơn trong dữ liệu\n")
cat("   - Nếu mục tiêu là hiểu rõ yếu tố nguy cơ: nên chọn Logistic Regression\n")
cat("   - Nếu mục tiêu là tối đa hóa độ chính xác dự đoán: nên so sánh cả hai và chọn mô hình tốt hơn\n")
cat("   - Trong trường hợp này,", better_acc, "có hiệu suất tổng thể tốt hơn\n")